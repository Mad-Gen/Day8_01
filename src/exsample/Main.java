...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    //       : そのメソッドを子クラスでオーバーライド(再定義もしくはカスタマイズ)して Cat.java とそれを継承した子クラスをSlackへポストしてください。
    ...
    // *****************************************
    // ★演習
    // Mini : Cat 型で宣言した変数に Lion　や自作の動物クラスを代入します。
    //      : その後　Cat　クラスで定義されている各種メソッドを呼び出し、オーバーライドによって動作が変わっていることを確認してみましょう。
    // Full : グループで「タクシー、電車、バス、飛行機...など」のグループを抽象化し、継承関係を持つ親クラスとして定義できるよう考えてみましょう。
